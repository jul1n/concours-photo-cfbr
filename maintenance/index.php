...
if (isset($_POST['action'])) {
    if (!$pdo) {
        ...
        if ($action === 'init_db') {
            try {
                $tables = [
                    ...
                    "jury_votes_analytics" => "CREATE TABLE IF NOT EXISTS jury_votes_analytics (
                        photo_id INTEGER,
                        jury_identifier TEXT,
                        score_aesthetic REAL,
                        score_theme REAL,
                        updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                        PRIMARY KEY (photo_id, jury_identifier)
                    )"
                ];
                ...
                $dirs = [
                    $root . '/data',
                    $root . '/data/confirmations',
                    $root . '/photos',
                    $root . '/photos/originals',
                    $root . '/photos/display_4k',
                    $root . '/photos/thumbs',
                    $root . '/photos/slides_optimized',
                    $root . '/_LOCAL_ONLY/original_photos/prizes',
                    $root . '/_LOCAL_ONLY/original_photos/slide'
                ];
                ...
                $messages[] = ['type' => 'success', 'text' => 'Base de données et dossiers système (data, photos, etc.) créés ou vérifiés avec succès.'];
            } catch (Exception $e) {
        ...
        // File Integrity Verification & Path Check
        if ($action === 'verify_integrity') {
            ...
            $dirsToCheck = [
                'data' => $root . '/data',
                'data/confirmations' => $root . '/data/confirmations',
                'photos' => $root . '/photos',
                'photos/originals' => $root . '/photos/originals',
                'photos/display_4k' => $root . '/photos/display_4k',
                'photos/thumbs' => $root . '/photos/thumbs',
                'photos/slides_optimized' => $root . '/photos/slides_optimized',
                'core' => $root . '/core',
            ...
            // 2. Critical Files (from check_paths.php)
            $filesToCheck = [
                'index.php' => $root . '/index.php',
                'upload.php' => $root . '/upload.php',
                'slideshow.php' => $root . '/slideshow.php',
                'core/db.php' => $root . '/core/db.php',
                'core/process_upload.php' => $root . '/core/process_upload.php',
                'includes/analytics.php' => $root . '/includes/analytics.php',
                'includes/pwa_loader.php' => $root . '/includes/pwa_loader.php',
                'data/concours.db' => $root . '/data/concours.db',
                'assets/logo_cfbr_100_ans.png' => $root . '/assets/logo_cfbr_100_ans.png',
                'fpdf/fpdf.php' => $root . '/fpdf/fpdf.php'
            ];
            foreach ($filesToCheck as $label => $path) {
                if (file_exists($path)) {
                    $report['files'][$label] = ['exists' => true];
                } else {
                    $suggest = '';
                    $dir = dirname($path);
                    if (is_dir($dir)) {
                        ...
                        foreach ($scanned as $f) {
                            if (strtolower($f) === strtolower($base)) {
                                $suggest = $f;
                                break;
                            }
                        }
                    }
                    $report['files'][$label] = ['exists' => false, 'suggest' => $suggest];
                }
            }
            ...
            try {
                if ($pdo) {
                    ...
                    foreach ($dbPhotos as $photo) {
                        $origPath = $root . '/photos/originals/' . $photo['filename_original'];
            ...
            // 4. Actual Write Test
            $testFile = $root . '/photos/originals/write_test_' . time() . '.txt';
            ...
            $messages[] = ['type' => 'success', 'text' => 'Vérification approfondie de l\'intégrité terminée.'];
        }
        ...
        if ($action === 'seed_dummies') {
            try {
                ...
                for ($i = 0; $i < 5; $i++) {
                    $fname = "[WEB] " . $firstnames[array_rand($firstnames)];
                    $lname = $lastnames[array_rand($lastnames)];
                    $email = strtolower($fname . "." . $lname . "." . rand(100, 999) . "@test-web.com");
                    $email = str_replace([' ', '[', ']'], '', $email);

                    // Alternate statuses to allow testing different phases
                    $statuses = ['pending', 'verified', 'approved'];
                    $status = $statuses[$i % 3];

                    // 1. Insert Participant
                    $stmt = $pdo->prepare("INSERT INTO participants (firstname, lastname, name, email, candidacy_type, is_verified, validation_status, address) 
                                           VALUES (?, ?, ?, ?, 'individual', 1, ?, 'Candidat Virtuel Internet')");
                    $stmt->execute([$fname, $lname, "$fname $lname", $email, $status]);
                    ...
                    $stmt->execute([
                        ...
                        $imgUrl,
                        "Barrage " . ($i + 1),
                        "Photo de démonstration récupérée depuis Unsplash.",
                        'cat1',
                        'approved'
                    ]);
                }
                $messages[] = ['type' => 'success', 'text' => '5 nouveaux participants avec photos Unsplash ajoutés avec succès !'];
            } catch (Exception $e) {
        ...
        if ($action === 'seed_from_bank') {
            try {
                $bankDir = realpath(__DIR__ . '/../_LOCAL_ONLY/original_photos/slide/');
                if (!$bankDir || !is_dir($bankDir)) {
                    throw new Exception("Le dossier source (banque) est introuvable. Vérifiez que '_LOCAL_ONLY' est présent à la racine. (Recherché : " . (__DIR__ . '/../_LOCAL_ONLY/original_photos/slide/') . ")");
                }
                ...
                foreach ([$uploadDirOriginal, $uploadDir4k, $uploadDirThumb] as $d) {
                    if (!is_dir($d)) {
                        mkdir($d, 0777, true);
                    }
                }
                ...
                $bankDir = realpath(__DIR__ . '/../_LOCAL_ONLY/original_photos/slide/');
                if (!$bankDir || !is_dir($bankDir)) {
                    throw new Exception("Le dossier source (banque) est introuvable. Utilisé : " . (__DIR__ . '/../_LOCAL_ONLY/original_photos/slide/'));
                }

                $participantsList = scandir($bankDir);
                foreach ($participantsList as $participantName) {
                    if ($participantName === '.' || $participantName === '..')
                        continue;

                    $participantPath = $bankDir . DIRECTORY_SEPARATOR . $participantName;

                    if (is_dir($participantPath)) {
                        $nameParts = explode(' ', $participantName);
                        // Remove leading dashes if any
                        $pureFname = ltrim($nameParts[0] ?? 'Participant', '-');
                        $pureLname = isset($nameParts[1]) ? implode(' ', array_slice($nameParts, 1)) : 'Inconnu';

                        $fname = "[BANQUE] " . $pureFname;
                        $lname = $pureLname;

                        $email = strtolower(str_replace([' ', '-'], '.', $participantName)) . "@seed.com";
                        $email = ltrim($email, '.');
                        ...
                        if ($existing) {
                            ...
                            // Alternate statuses to allow testing different phases
                            $statuses = ['pending', 'verified', 'approved'];
                            $status = $statuses[$seededCount % 3];

                            $stmt = $pdo->prepare("INSERT INTO participants (firstname, lastname, name, email, candidacy_type, is_verified, validation_status, address) 
                                                   VALUES (?, ?, ?, ?, 'individual', 1, ?, 'Données de test')");
                            $stmt->execute([$fname, $lname, "$fname $lname", $email, $status]);
                            $pid = $pdo->lastInsertId();
                            $seededCount++;
                        }

                        // 2. Scan photos in folder
                        $photosList = scandir($participantPath);
                        foreach ($photosList as $photoName) {
                            if ($photoName === '.' || $photoName === '..')
                                continue;

                            $photoPath = $participantPath . DIRECTORY_SEPARATOR . $photoName;
                            if (is_file($photoPath)) {
                                $ext = strtolower(pathinfo($photoName, PATHINFO_EXTENSION));
                                if (in_array($ext, ['jpg', 'jpeg', 'png', 'webp'])) {

                                    // Check if photo already in DB for this participant
                                    $stmt = $pdo->prepare("SELECT id FROM photos WHERE participant_id = ? AND title = ?");
                                    $stmt->execute([$pid, "Photo " . $photoName]);
                                    if ($stmt->fetch()) {
                                        $skippedPhotos++;
                                        continue;
                                    }
                                    ...
                                    $newBaseName = "seed_" . $pid . "_" . $randomHash;
                                    ...
                                    if (copy($photoPath, $uploadDirOriginal . $fileOriginal)) {
                                        ...
                                        $stmt = $pdo->prepare("INSERT INTO photos (participant_id, filename_original, filename_4k, filename_thumb, width, height, title, category, status) 
                                                           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
                                        $stmt->execute([$pid, $fileOriginal, $file4k, $fileThumb, $width, $height, "Photo " . $photoName, 'cat1', 'approved']);
                                        $photoCount++;
                                    }
                                }
                            }
                ...
                if ($seededCount === 0) {
                    // Fallback to dummy internet seeding if bank is empty
                    $resMsgFallback = "Dossier banque vide. Lancement du peuplement via Internet (Dams)... ";
                    $firstnames = ['Alex', 'Morgan', 'Taylor', 'Jordan', 'Casey', 'Robin'];
                    $lastnames = ['Durand', 'Petit', 'Lefebvre', 'Moreau', 'Fournier', 'Roux'];
                    $damImages = [
                        'https://images.unsplash.com/photo-1590732152848-1da50906f0f7?auto=format&fit=crop&q=80&w=1200',
                    ...
                    for ($i = 0; $i < 5; $i++) {
                        $fname = "[BANQUE-WEB] " . $firstnames[array_rand($firstnames)];
                        $lname = $lastnames[array_rand($lastnames)];
                        $email = strtolower($fname . "." . $lname . "." . rand(100, 999) . "@seed-bank.com");
                        $email = str_replace([' ', '[', ']'], '', $email);
                        ...
                        $stmt = $pdo->prepare("INSERT INTO participants (firstname, lastname, name, email, candidacy_type, is_verified, validation_status, address) 
                                               VALUES (?, ?, ?, ?, 'individual', 1, ?, 'Données de test Web')");
                        $stmt->execute([$fname, $lname, "$fname $lname", $email, $status]);
                        $pid = $pdo->lastInsertId();
                        ...
                        $stmt = $pdo->prepare("INSERT INTO photos (participant_id, filename_original, title, description, category, status) 
                                               VALUES (?, ?, ?, ?, ?, ?)");
                        $stmt->execute([$pid, $imgUrl, "Barrage Web " . ($i + 1), "Photo récupérée sur Unsplash (Thème Barrage).", 'cat1', 'approved']);
                        $seededCount++;
                        $photoCount++;
                    }
                    $messages[] = ['type' => 'info', 'text' => $resMsgFallback];
                }

                $resMsg = "Seeding terminé : $seededCount nouveaux participants, $photoCount nouvelles photos.";
                if ($skippedParticipants > 0 || $skippedPhotos > 0) {
                    $resMsg .= " ($skippedParticipants participants et $skippedPhotos photos ignorés car déjà présents)";
                }
                $messages[] = ['type' => 'success', 'text' => $resMsg];
            } catch (Exception $e) {
                $messages[] = ['type' => 'danger', 'text' => "Erreur Seeding Banque: " . $e->getMessage()];
            }
        }
...
// Check existing tables for status monitoring (RECALCULATED AFTER ACTIONS)
$existingTables = [];
if ($pdo) {
    try {
...
<body class="bg-gray-100 min-h-screen text-slate-800">
    ...
    <div class="max-w-4xl mx-auto px-4 py-12">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
            <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                <h3 class="font-bold text-slate-700 mb-4 flex items-center text-sm uppercase tracking-wider">
                    <i class="fas fa-table mr-2 text-slate-400"></i> État des Tables SQL
                </h3>
                <div class="space-y-2">
                    <?php
                    $required = ['participants', 'photos', 'jury_members', 'jury_tokens', 'jury_votes_analytics', 'settings', 'analytics', 'votes_tour1', 'votes_tour2'];
                    foreach ($required as $t):
                        $ok = in_array($t, $existingTables);
                        ?>
                        <div class="flex justify-between items-center text-xs">
                            <span class="<?= $ok ? 'text-slate-600' : 'text-red-500 font-bold' ?>"><?= $t ?></span>
                            <span
                                class="px-2 py-0.5 rounded text-[10px] font-bold uppercase <?= $ok ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700 animate-pulse' ?>">
                                <?= $ok ? 'OUI' : 'MANQUANTE' ?>
                            </span>
                        </div>
                    <?php endforeach; ?>
            ...
            <div
                class="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex flex-col justify-center text-center">
                <div class="text-xs text-slate-400 uppercase font-bold mb-3">Recommandation Système</div>
                <?php if (count(array_intersect($required, $existingTables)) < count($required)): ?>
            ...
            <section class="space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="grid grid-cols-1 gap-3">
                        ...
                        <!-- Seeding Banque Locale -->
                        <div class="space-y-2">
                            <form method="POST">
                                <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
                                <input type="hidden" name="action" value="seed_from_bank">
                                <button type="submit"
                                    class="w-full text-left px-4 py-3 bg-purple-50 hover:bg-purple-100 border border-purple-100 rounded-xl text-xs font-bold flex items-center justify-between group transition-all">
                                    <span class="flex items-center gap-3">
                                        <i class="fas fa-images text-purple-600 text-lg"></i>
                                        <span>Peupler (Banque Locale _LOCAL_ONLY)</span>
                                    </span>
                                    <i
                                        class="fas fa-chevron-right text-purple-300 group-hover:translate-x-1 transition-transform"></i>
                                </button>
                            </form>

                            <form method="POST">
                                <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
                                <input type="hidden" name="action" value="seed_dummies">
                                <button type="submit"
                                    class="w-full text-left px-4 py-3 bg-indigo-50 hover:bg-indigo-100 border border-indigo-100 rounded-xl text-xs font-bold flex items-center justify-between group transition-all">
                                    <span class="flex items-center gap-3">
                                        <i class="fas fa-globe text-indigo-600 text-lg"></i>
                                        <span>Peupler avec Photos Internet (Unsplash Barrages)</span>
                                    </span>
                                    <i
                                        class="fas fa-chevron-right text-indigo-300 group-hover:translate-x-1 transition-transform"></i>
                                </button>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            ...
                            <ul class="space-y-2">
                                ...
                                <?php endforeach; ?>
                                <?php foreach ($integrityReport['files'] as $name => $data): ?>
                                    <li class="flex flex-col text-sm py-1 border-b border-gray-50 last:border-0">
                                        <div class="flex justify-between items-center">
                                            <span class="text-slate-600">Fichier: <code
                                                    class="bg-gray-100 px-1 rounded"><?= $name ?></code></span>
                                            <i
                                                class="fas <?= $data['exists'] ? 'fa-check-circle text-emerald-500' : 'fa-times-circle text-red-500' ?>"></i>
                                        </div>
                                        <?php if (!$data['exists'] && !empty($data['suggest'])): ?>
                                            <div class="text-[10px] text-amber-600 font-bold mt-1 bg-amber-50 px-2 py-1 rounded">
                                                <i class="fas fa-lightbulb mr-1"></i> Suggestion : Un fichier existe avec une casse
                                                différente : <code><?= $data['suggest'] ?></code>
                                            </div>
                                        <?php endif; ?>
                                    </li>
                                <?php endforeach; ?>
    ...
    <footer class="mt-12 mb-12 text-center text-gray-400 text-sm border-t pt-8 max-w-4xl mx-auto">
        <div class="flex flex-wrap justify-center gap-8">
            <a href="../index.php" class="hover:text-blue-500 transition-colors flex items-center gap-2 font-semibold">
                <i class="fas fa-home"></i> Accueil Site
            </a>
            <a href="../jury/login.php"
                class="hover:text-blue-500 transition-colors flex items-center gap-2 font-semibold">
                <i class="fas fa-gavel"></i> Espace Jury
            </a>
            <a href="../upload.php" class="hover:text-blue-500 transition-colors flex items-center gap-2 font-semibold">
